...
\textbf{(2)} Fetch Salesforce account, then user/opportunity/contract/CLM/invoice in parallel
\textbf{(3)} \texttt{validate\_business\_rules} (5 domains) + \texttt{check\_financial\_alignment} (cross-currency)
\textbf{(4)} PROCEED $\rightarrow$ provision; BLOCK/ESCALATE $\rightarrow$ notify; generate reports.
}
\end{minipage}
...
\caption{Architecture: triggers, Pydantic AI agent orchestration with 16 tools, FastMCP servers, and external system integrations.}
\end{figure}
...
The agent leverages \textbf{OpenAI GPT-4o-mini} (configurable via \texttt{OPENAI\_MODEL}) with \textbf{Pydantic AI} for native tool calling. The LLM autonomously decides which of the 16 registered tools to call, in what order, and how many times. Falls back to \textbf{Ollama} local models when no API key is available.
...
\begin{minipage}[t]{0.54\textwidth}
\vspace{0pt}
{\scriptsize\textit{Note: All 16 agent tools are currently mirrored as FastMCP server definitions in \texttt{app/mcp/} (7 servers: Salesforce, CLM, NetSuite, Currency, Provisioning, Notifications, Validation). These can be extracted to standalone MCP services for multi-agent collaboration.}}
...
\vspace{0.15em}
\textbf{Multi-Agent Interoperability}: All 16 agent tools are mirrored as FastMCP server definitions (7 servers in \texttt{app/mcp/}), separating agent logic from external system access. These can be extracted to standalone MCP services with independent versioning, access control, and audit logging. The A2A protocol provides a standardized layer for agent-to-agent collaboration without exposing internal state across boundaries.
}
