...
1. **~~Task Monitor Agent~~** ✅ **PARTIALLY IMPLEMENTED**: Risk and sentiment assessment is now a **mandatory step** in the onboarding workflow for all PROCEED accounts (step 5). After provisioning, the agent always calls `check_onboarding_progress`, `identify_onboarding_risks`, and `get_customer_sentiment` — ensuring consistent risk detection across individual and batch runs. The `identify_onboarding_risks` tool detects overdue tasks, customer inactivity (no login after 3 days), SSO not configured after kickoff, blocked tasks, stalling onboardings (<30% after 7 days), and customer sentiment decline. Sentiment scoring uses a **DistilBERT transformer model** (`distilbert-base-uncased-finetuned-sst-2-english`) via PyTorch + HuggingFace for ML inference, with keyword-based fallback. The `send_task_reminder` tool sends reminders to task owners. The `check_onboarding_progress` tool provides a dashboard view with health status (on_track/at_risk/stalled) and integrates sentiment signals — negative sentiment shifts health to at_risk even when task metrics look normal. The CS assistant agent can also be asked to check progress, identify risks, inspect sentiment, and take corrective action conversationally. See `app/integrations/provisioning.py`, `app/integrations/sentiment.py`, and `app/agent/onboarding_agent.py`. **Remaining enhancement**: Scheduled cron job for hourly automated checks without CS prompting.
