...
“This is our FastAPI backend, which acts as the control layer for the onboarding agent.

At the top, we have the Webhooks section, which is our production entry point.
So when Salesforce detects that an Opportunity has moved to Closed Won, it automatically calls our onboarding webhook. We also expose a debug endpoint so we can safely test the same flow with custom payloads during development.

Next is the Demo section, which is what we’ll be using today.
These endpoints let us trigger onboarding for a single account, run batch scenarios, simulate random API failures to test resilience, inspect the onboarding task checklist, and review any reports generated by the agent.

We also expose a standard health endpoint, which is used for monitoring and operational checks.

Behind all of these endpoints is a LangGraph-based state Agent.
This is what actually orchestrates the workflow. This includes pulling the data from Salesforce CRM, CLM Tool, and NetSuite ERP, validating business rules, analyzing risk with an LLM, making decisions, and then executing the appropriate actions end-to-end.”**
...
**Total: ~895 words ≈ 5-6 minutes at conversational pace**
