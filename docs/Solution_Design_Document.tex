...
\item \texttt{email\_*.html} -- Professional HTML email template
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{04_state_machine.png}
    \vspace{-0.6em}
\end{figure}
...
\item \textbf{No transient retry}: API failures (5xx, timeouts) halt immediately without retry/backoff
...
\texttt{DELETED-004} & Invalid Account & Account marked \texttt{IsDeleted=true} & BLOCK \\
